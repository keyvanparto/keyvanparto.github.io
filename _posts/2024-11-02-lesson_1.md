...
Our goal is to build a learning machine that, using historical data (the past), can make reliable predictions about the future. Given an input $$ x $$, possibly never seen before, we want to obtain an estimate $$ \hat{y} $$ that is as close as possible to the actual value of $$ y $$ from the original system.
...
- categorical variables: $$ \{ \mathcal{Y, G, B} \} $$, which belong to a specific category (like colors);
- graphs.
...
- If $$ x \in \mathbb{R}^d $$ (a vector of real numbers with dimension $$ d $$), the distance between $$ x_1 $$ and $$ x_2 $$ is calculated using the norm:
...
where $$ \mathcal{Y}, \mathcal{G}, \mathcal{B} $$ represent the colors yellow, green, and blue, respectively. This implies that $$ Y $$ is closer to $$ G $$ than to $$ B $$, which is incorrect.

<blockquote class="prompt-danger">
Although one might naturally think that colors share similarities, in the case of categories, colors are merely labels, and any perceived similarity between them does not reflect an actual closeness or association.
</blockquote>

Therefore, we transform these elements into a representation similar to the previous cases.
...
But our journey is just beginning. The next lessons will bring us closer to discovering how to build and refine a *learning machine* capable of learning, adapting, and even predicting something similar to the future. And, as we’ve seen, although it will never be exactly identical, isn’t it perhaps the uncertainty, the lack of a definitive answer, that makes our journey so meaningful?
